...
public class J_MailServiceImpl implements J_MailService {
    ...
    private MimeMessage createEmail(String email, String token) throws MessagingException {
        // 매일 내용을 생성하는 메서드
        // : 매일 주소와 JWT 토큰을 매개변수로 받음

        MimeMessage message = javaMailSender.createMimeMessage();
        message.setFrom(sederEmail); // 환경변수의 이메일 (발신자)
        message.setRecipients(MimeMessage.RecipientType.TO, email);

        message.setSubject("=== [K5_iot_springboot] 이메일 인증 링크 방송 ===");

        String body = """
                    <h3>이메일 인증링크 입니다.</h3>
                    <a href="http://localhost:8080/api/v1/auth/verify?token=%s"> 여기를 클릭하여 인증을 완료해주세요
                """.formatted(token);
    ...
    public void sendEmail(MailRequest.@Valid SendMail req) {
        ...
        try {
            ...
            System.out.println("인증 이메일이전송되었습니다.");
