...
public class C_BookServiceImpl implements C_BookService {
    ...
    public ResponseDto<BookResponseDto> createBook(BookCreateRequestDto dto) {
        ...
        try {
            C_Book newBook = new C_Book(
                    null, dto.getWriter(), dto.getTitle(),
                    dto.getContent(), dto.getCategory()
            );

            C_Book saveBook = bookRepository.save(newBook);

            return ResponseDto.setSuccess("책이 생성되었습니다.", toResponse(saveBook));

        } catch (Exception e) {
            return ResponseDto.setFailed("책이 등록 중 문제가 발생하였습니다: " + e.getMessage());
    ...
    public ResponseDto<List<BookResponseDto>> getAllBooks() {
        List<BookResponseDto> date = null;
        date = bookRepository.findAll()
                .stream()
                .map(this::toResponse)
    ...
    public ResponseDto<BookResponseDto> getBookById(Long id) {
        try {
            ...
            return ResponseDto.setFailed("책이 조회 중 문제가 발생하였습니다: " + e.getMessage());
        }
    }
    ...
    public ResponseDto<BookResponseDto> updateBook(Long id, BookUpdateRequestDto dto) {
        try {
            ...
            return ResponseDto.setFailed("책이 수정 중 문제가 발생하였습니다: " + e.getMessage());
        }
    }
    ...
    public ResponseDto<Void> deleteBook(Long id) {
        try {
            ...
            return ResponseDto.setFailed("책 삭제 중 문제가 발생 하였습니다.");
        }
    }
    ...
    public ResponseDto<List<BookResponseDto>> getBooksByTitleContaining(String keyword) {
        ...
        if (found.isEmpty()) {
            return ResponseDto.setFailed("검색 결과가 없습니다.");
        }
        data = found.stream()
                .map(this::toResponse)
                .collect(Collectors.toList());
