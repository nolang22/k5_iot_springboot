...
//  아무런 매개변수가 없는 기본 생성자를 Protected 접근 제어자로 생성
...
public class H_Article extends BaseTimeEntity {
    ...
    // 작성자 (작성자:게시글 = 1:N) //
    @ManyToOne(fetch = FetchType.LAZY, optional = false) // 현재 페이지는 N 이기에 N:1
    // fetch 가져오다. LAZY 게으름
    // 연관된 엔티티를 필요한 메인 DB 에서 로딩
    @JoinColumn(name = "author_id",
            foreignKey = @ForeignKey(name = "fk_)articles_author"))
