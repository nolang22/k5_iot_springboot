...
public enum C_Category {
    ...
    /*  1) JSON 직렬화 시 종작 메서드
            : Enum >> String
            : Enum 객체를 JSON 응답으로 변환할 떄, dbValue 값을 그대로 전달
    ...
    /*  2) JSON 역직렬화 시 종작 메서드
            : String >> Enum
            : JSON 요청 값을 Enum으로 변환할 떄, Enum 이름(name)과 DB 값(dbValue) 모두 인식
    ...
    public static C_Category fromDbValue(String dbValue) {
        ...
        return Arrays.stream(values())
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Unknow category DB: " + dbValue));
    }
}
