...
public class D_Post {
    ...
    @OneToMany(
            ...
            // : 부모(D_Post)에 대한 persist/merge/remove 등이 자식(D_Comment_로 전이
            // - 게시글 지정/삭제 시 댓글도 같이 처리
            orphanRemoval = true,
    ...
    /*  연관 관계 메서드
            ...
            cf) 영속성 컨텍스트
                : JPA에서 엔터티 객체를 연구, 저장하는 환경을 의미
                - 엔티티 매니저에 의해 관리, save(), remove(), find()와 같은 작업을 수행
                - 영속성 컨텍스트에 저장된 언티티는 1차 개시에 보관
    ...
    public void addComment(D_Comment comment) {
        ...
        if (!this.comments.contains(comment)) {
            // : 기존의 댓글 배열(comment)에 추가하려는 댓글(comment)이 포함되어 잇지 않은 경우
