...
/*  === JwtAuthenticationFilter ===
    : JWT 인증 필터
    - 요청에서 JWT 토큰을 추출
        >> request의 header 에서 토큰을 추출하여 검증 (유효한 경우 SecurityContext에 인증 정보 설정)
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            // 0) 사전 스킵 조건: 이미 인증된 컨택스트가 있으면 그대로 진행(스킵) (다른 필터가 인증처리를 한 경우, 중복 인증 방지)
            if (SecurityContextHolder.getContext().getAuthentication() != null) {
                // 현재 스레드(요청) 컨텍스트에 이미 정보가 들어있는지 확인
            ...
            //      >> 해당 시점부터 현재 요청은 "username이라는 사용자가 authorities 권한으로 인증된" 상태가 됨
    ...
    private void setAuthenticationContext(
        ...
        // 빈 SecurityContext 개게 생성 = 인증 토큰 주입
    ...
    /* USERS/ADMIN -> "ROLE_USER"/"ROLE_ADMIN" 으로 매핑 */
