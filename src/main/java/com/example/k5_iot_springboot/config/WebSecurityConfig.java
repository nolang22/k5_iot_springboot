...
/*  === WebSecurityConfig ===
    : 스프링 시큐리티 전체 규칙 설정
    : Spring Security를 통해 웹 애플리케이션의 보안을 구성 (보한 환경 설정)
...
public class WebSecurityConfig {
    private final JwtAuthenticationFilter jwtAuthenticationFilter; // 사용지 정의 JWT 검증 필터 (아래에서 필터 체인에 추가)
    ...
    /*  =================
        ...
        cf) CORS (Cross Origin Resource Sharing)
            ...
            - REST API 사용 시 다른 출러(도메인)에서 API에 접근 할 수 었도록 허용하는 정책
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        CorsConfiguration config = new CorsConfiguration(); // 출처/헤더/메서드/쿠기 허용 등을 담는 CORS 정책 객체

        List<String> origins = splitToList(allowedOrigins);

        // 1) 인증 정보(쿠키/Authorization) 허용
    ...
    /*  =================
        Security Filter Chain
            : 보안 필터 페인 정의 ,특정 HTTP 요청에 대한 웹 기반 보안 구성
