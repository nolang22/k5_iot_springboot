...
/*  === 스프링 부트 계층 ===
    ...
    1) Controller(Presentation 계층)
        : HTTP 요청을 받음
        - 입력 DTO를 검증(@Valid)하고, 비즈니스 로직은 전부 Service에 위임
        - 일관된 응답 포맷으로 감싸서 반환 (ResponseEntity<ResponseDto<?>>)

    2) Service(Business 계층)
        : 트랜잭션 경계(@Transaction)가 잡히는 곳
        - 도메인 규칙(중복 검사, 상태 전이, 관한 확인 등)을 처리
        - Repository를 통해 DB에 접근
        +) DTO <<->> 엔터티 매핑도 발생
        +) createdAt/updatedAT은 엔터티의 @PrePersist/@PreUpdate 대신 서비스 레벨에서 관리
