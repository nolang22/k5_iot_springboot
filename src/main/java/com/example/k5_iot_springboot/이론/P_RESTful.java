...
/*  === RESTful API ===
    ...
    REpresentaion State Transfer
        : 자원(Resource)를 URI로 식별하고
            , HTTP 메서드를 이용해 해당 지원에 대한 행위를 정의하는 아키텍처 스타일
        - 자원의 표현: JSON, XML 등
        - 상태 전달: 요청(Request)과 응답(Response)으로 상태 전송
        - HTTP 표준 메서드를 활용

        >> 네트워크 상에서 클라이언트와 서버 간의 통신 방식 중 하나
    ...
    케이스1) 사용자/인증
        ...
        - 내 정보 조회: GET /api/v1/users/me (로그인 한 자시의 정보 조회)
    ...
    케이트3) 게시판
        - 게시글 목록: GET /api/v1/posts?category=notice&search=java&page=1&size=id
