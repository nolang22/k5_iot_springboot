...
/*  === 스프링 시큐리티(Spring Security, 보안) ===
    ...
    cf) CSRF(Cross-Site Request Forgery)
        : 자신(인증된 사용자)의 의자와는 상관없이 특정 웹 애플리케이션에
            , 공격자가 원하는 요청을 보내도록 유도하는 웹 보안 취약점

    === Security 사용 목적 ===
    1. 보안 기본키 자동 제공: 비밀번호 암호화, 세션/토큰 인증, 권한 체크, CSRF 보호 등
    2. 표준화된 구조: 보안 코드가 일관되고 유지보수 쉬움
    3. 유연한 확장성: 소셜 로그인, OAuth2, JWT, 2FA 등으로 확장 가능
    4. 필수 방어방: 인증/인가 없는 서비스는 보안 취약
    ...
    5. Authority (권한), ROLE(역할)
        - 역할에 따라 권한이 달라짐
        >> 해당 요청(해당 URL)은 권한 ROLE_ADMIN만 들어갈 수 있음의 규칙을 정의하는 것
    6. SecurityContext
        : 현재 요청의 인증/권한 저장
        - 현재의 사용자 기록
    7. Filter Chain
    ...
    >> 모든 요청은 필터 체인을 통과
    >> 인증이 필요한 URL 이면 로그인 정보 확인 + 성공 시 SecurityContext에 사용자/권한을 저장

    === Spring Security 전체 처리 절차 ===
    1) HTTP 요청
        : 사용자가 브라우저/앱에서 요청(예: /login)을 보냄
            +) 데이터나 인증 정보(JWT 토큰)가 포함되어 있을 수 도 있고, 없을 수도 있음
