...
/*  === JpaRepository<Em ID> ===
    : Spring Date JPA에서 제공하는 기본 인터페이스
    - CRUD와 페이징, 정렬을 포함한 다향한 데이터 엑서스 메서드를 제공
    - 반환타입 메서드명(매개변수);의 추상 메서드 형식

    1. CRUD 메서드
        1) E save(E entity);
            : 새로운 엔티티를 저장하거나, 기존 엔티티를 업데이트
            - 매개변수의 데이터값 중 Id값이 DB에 존재하는 경우 update / 존재하지 않을 경우 insert
