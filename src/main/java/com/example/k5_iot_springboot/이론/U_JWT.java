...
/*  === JWT (JSON Web Token) === //
    : JSON 객체를 이용하여 인증 정보를 인전하게 전달하는 토큰 기반 인증 방식
    - 서버가 클라이언트(사용자)에게 발급하는 다지털 서명이 된 토큰
    ...
    1. JWT와 인증/인가
        ...
        2) 인가
            : JWT의 사용자 권한 정보(ROLE, AUTHORITY 등)가 포함될 수 있음
            - 서버가 JWT를 해석해서 해당 사용자가 어떤 API나 리소스에 접근할 수 있는지 판단
