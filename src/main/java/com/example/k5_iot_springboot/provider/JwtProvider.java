...
cf) JWT
    ...
    - (클라이언트가)서버에 요청할 떄마다 전달 가능 (사용자 정보 확인용, Authorization: Bearer <토큰>)
...
public class JwtProvider {
    ...
    // === 상수 & 필드 선언 === //
    /* HTTP Header의 Authorization의 접두사 */
    ...
    public String generateJwtToken(String username, Set<String> roles) {
        ...
        // List로 변환하여 직렬화 시 타임 안정성 확보
        List<String> roleList = (roles == null) ? List.of() : new ArrayList<>(roles);
        return Jwts.builder()
                // 표준 클레임 sub(Subject)에 사용자 아이디(또는 고유 식별자) 설정
                .setSubject(username) //페이로드 저장
                .claim(CLAIM_ROLES, roleList) // 커스텀 클레임 키에 권한 목록 저장, 페이로드 저장
    ...
    private Claims parseClaimsInternal(String token, boolean allowClockSkewOnExpiry) {
        // allowClockSkewOnExpiry: 만료 직후 허용 오지 적용 여부
        try {
            ...
            // : 만료 시간이 지난 토큰에도(예외 안에도) Claims 정보가 들어있다
            if (allowClockSkewOnExpiry && clockSkewSeconds > 0 && ex.getClaims() != null) {
                // 호출부가 "만료 직후 와 허용"을 활성화했고, 설정값이 0보다 큰지 확인
    ...
    /* roles >> Set<String> 변환 */
    @SuppressWarnings("unchecked") // 제네릭 캐스팅 경고 억제 (런타림 타입 확인으로 보완)
    public Set<String> getRolesFromJwt(String tokenWithoutBearer) {
        // get("roles")로 커스텀 클레임을 가져오면, JSOM 파싱 결과가 List로 반환이 일반적
