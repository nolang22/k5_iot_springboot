...
public class I_OrderController {
    ...
    public ResponseEntity<ResponseDto<OrderResponse.Detail>> create(
            // 메게변수 - controller (@PathVariable, @RequestBody, @RequestParam)
        ...
        // return ResponseEntity.ok(response); // 간결하게 사용
        return ResponseEntity.ok().body(response); // 보여줄게 있을때

    }

    // 주문 승인: USER 불가, ADMIN/MANAGER만 가능
